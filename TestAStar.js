...
function aStarSearch(graph, start, goal, heuristics, timeout) {
    ...
    var costDict = new collections.Dictionary();
    var fValDict = new collections.Dictionary();
    var parentDict = new collections.Dictionary();
    ...
    var frontier = new collections.PriorityQueue(function (a, b) {
        var fa = fValDict.containsKey(a) ? fValDict.getValue(a) : Infinity;
        var fb = fValDict.containsKey(b) ? fValDict.getValue(b) : Infinity;
        if (fa === fb)
            return 0;
        return fa < fb ? 1 : -1;
    });
    ...
    while (!frontier.isEmpty()) {
        ...
        for (var _i = 0, _a = graph.outgoingEdges(current); _i < _a.length; _i++) {
            var edge = _a[_i];
            var next = edge.to;
            var costNext = costDict.getValue(current) + edge.cost;
            var fVal = costNext + heuristics(next);
...
function followParent(start, goal, parentDict) {
    var current = goal;
    var path = [goal];
    while (current != start) {
